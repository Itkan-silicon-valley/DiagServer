...
/**
 * Telemetry wrapper that mirrors SDK telemetry into the schema-driven diag bus.
 *
 * This lets existing code keep using telemetry.addData()/update() while also
 * publishing those fields to the DiagBus.
 */
public class DiagTelemetry implements Telemetry, AutoCloseable {
    private static final String DEFAULT_NUMBER_FORMAT = "%.3f";
    private final Telemetry delegate;
    private final DiagBus bus;
    private final Map<String, ValueProvider> captured = new LinkedHashMap<>();
    private boolean busStarted;

    public DiagTelemetry(Telemetry delegate, DiagBus bus) {
